...
VDD = 0.5
TEMP = 25
SLEWS = [20e-12, 50e-12, 100e-12, 200e-12, 500e-12]   # 5 slew values
LOADS = [0.1e-15, 0.5e-15, 1.0e-15, 2.5e-15, 10.0e-15]  # 5 load values
...
def gen_liberty_pin(cell_name, pin_name, results, cell_info):
    ...
    lines.append(f'        cell_rise (delay_5x5) {{')
    ...
    lines.append(f'        cell_fall (delay_5x5) {{')
    lines.append(f'          index_1 ("{", ".join(f"{s*1e9:.4f}" for s in SLEWS)}");')
    lines.append(f'          index_2 ("{", ".join(f"{l*1e12:.4f}" for l in LOADS)}");')
    lines.append(fmt_table(results, 'cell_fall'))
    lines.append(f'        }}')

    lines.append(f'        rise_transition (delay_5x5) {{')
    lines.append(f'          index_1 ("{", ".join(f"{s*1e9:.4f}" for s in SLEWS)}");')
    lines.append(f'          index_2 ("{", ".join(f"{l*1e12:.4f}" for l in LOADS)}");')
    lines.append(fmt_table(results, 'rise_transition'))
    lines.append(f'        }}')

    lines.append(f'        fall_transition (delay_5x5) {{')
    lines.append(f'          index_1 ("{", ".join(f"{s*1e9:.4f}" for s in SLEWS)}");')
    lines.append(f'          index_2 ("{", ".join(f"{l*1e12:.4f}" for l in LOADS)}");')
