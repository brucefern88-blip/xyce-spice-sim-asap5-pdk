#!/usr/bin/env python3
"""Generate ASAP5 5nm XNOR2x1 GDS layout using gdstk.

10T: XOR2x1 (8T) + output inverter (2T).
  INV_B (CPP1) + INV_A (CPP2) + TG1 (CPP3) + TG2 (CPP4) + INV_OUT (CPP5)
  TG outputs -> internal node X, INV_OUT produces Y = ~X = ~(A^B) = XNOR.
  2 fins per device (x1 drive strength).
...
# --- 3. FINS (2 per device for x1 strength) ---
nmos_fins = [25, 49]
pmos_fins = [91, 115]
...
# CPP5 center=198: INV_OUT (inverts X -> Y)
cell.add(gdstk.rectangle((190, 5), (206, 135), layer=L_POLY))

NY1, NY2 = 18, 56   # NMOS SDT y range
PY1, PY2 = 84, 122  # PMOS SDT y range
...
# SD5 (x=120-130): TG1 drain / TG2 source = X (internal XOR output)
...
# X internal node: connect SD5 and SD7 (TG outputs), then to INV_OUT gate
# SD5 vertical strap
cell.add(gdstk.rectangle((122, NY2), (128, PY1), layer=L_M1))
# SD7 vertical strap
cell.add(gdstk.rectangle((166, NY2), (172, PY1), layer=L_M1))

# INV_OUT (CPP5): VSS and VDD for inverter
# INV_OUT shares SD7 (x=164-174) as source on left side -- wait, need to
# reconsider. INV_OUT gate is at CPP5 (x=190-206). Its source is the
# diffusion between CPP4 and CPP5 (which is also SD7/TG2 drain = X node).
# Actually SD7 is between CPP4 and CPP5, so:
#   - For TG2: SD7 is the drain (X node)
#   - For INV_OUT: the left diffusion (shared with TG2) connects X to the gate
# Wait -- that's wrong. The diffusion between TG2 drain and INV_OUT source
# is actually the X node connection done through the diffusion itself.
# SD7 (x=164-174) serves as both TG2 drain and INV_OUT source (both = X).
# SD8 (x=208-218) is INV_OUT drain = Y.

# INV_OUT source (SD7) connects to X -- already handled by diffusion sharing.
# INV_OUT NMOS source connects to X, but for INV we need VSS on source side.
# Correction: INV_OUT is a proper inverter:
#   NMOS: gate=X, source=VSS, drain=Y
#   PMOS: gate=X, source=VDD, drain=Y
# The source of INV_OUT NMOS needs VSS, not X. So SD7 is NOT shared.
# Between CPP4 (x=146-162) and CPP5 (x=190-206), there's a gap from 162 to 190.
# We need two SD contacts there:
#   SD7a (x=164-174): TG2 drain = X
#   SD7b (x=178-188): INV_OUT source = VSS (NMOS), VDD (PMOS)
# Let me place them properly.

# Remove the SD7 we already added and replace:
# (SD7 was already added above as x=164-174 for TG2 drain = X)
# Add SD7b for INV_OUT source
cell.add(gdstk.rectangle((178, NY1), (188, NY2), layer=L_SDT))   # INV_OUT NMOS src=VSS
cell.add(gdstk.rectangle((178, PY1), (188, PY2), layer=L_SDT))   # INV_OUT PMOS src=VDD
cell.add(gdstk.rectangle((178, NY1), (188, NY2), layer=L_COSD))
cell.add(gdstk.rectangle((178, PY1), (188, PY2), layer=L_COSD))
...
# X node: connect SD5, SD7, and INV_OUT gate via M1/M2
# INV_OUT gate COG pad on M1
cell.add(gdstk.rectangle((190, 60), (206, 75), layer=L_M1))

# Gate contact pads for inverters
cell.add(gdstk.rectangle((14, 60), (30, 75), layer=L_M1))
cell.add(gdstk.rectangle((58, 60), (74, 75), layer=L_M1))

# TG split gate COG pads
cell.add(gdstk.rectangle((102, 50), (118, 58), layer=L_M1))
cell.add(gdstk.rectangle((102, 72), (118, 80), layer=L_M1))
cell.add(gdstk.rectangle((146, 50), (162, 58), layer=L_M1))
cell.add(gdstk.rectangle((146, 72), (162, 80), layer=L_M1))
...
# X node: SD5 -> SD7 -> INV_OUT gate (all connected via M2)
cell.add(gdstk.rectangle((122, 62), (128, 68), layer=L_V1))   # X at SD5
cell.add(gdstk.rectangle((166, 82), (172, 88), layer=L_V1))   # X at SD7
cell.add(gdstk.rectangle((194, 62), (200, 68), layer=L_V1))   # X at INV_OUT gate
# Y output
cell.add(gdstk.rectangle((210, 62), (216, 68), layer=L_V1))   # Y via
...
# B to TG2 P-gate (included in BB M2 run above with separate via)
# X node: SD5 -> SD7 -> INV_OUT gate
cell.add(gdstk.rectangle((122, 82), (172, 88), layer=L_M2))   # X horizontal upper
cell.add(gdstk.rectangle((166, 62), (200, 68), layer=L_M2))   # X horizontal to INV_OUT gate
# Y output stub
...
# --- Write GDS ---
outpath = "/Users/bruce/CLAUDE/asap5/charflow/gds/XNOR2x1.gds"
lib.write_gds(outpath)
print(f"XNOR2x1.gds written to {outpath}")
